...
import { 
  ...
  BookOpen, ChevronRight, Menu, X, Download, Search, Activity, Database
} from 'lucide-react';

// --- KONFIGURASI SUPABASE ---
// Masukkan URL dan Anon Key dari Project Supabase Anda di sini
...
export default function App() {
  ...
  // Fetch Data Logic (MIGRASI KE SUPABASE)
  useEffect(() => {
      ...
      try {
        if (SUPABASE_URL && SUPABASE_ANON_KEY) {
          ...
          if (supabaseData && supabaseData.length > 0) {
            ...
            console.warn("Tabel Supabase kosong, menggunakan mock data.");
            setData(mockGtkData);
          }
        } else {
          // Jika URL/Key kosong, simulasi loading dan pakai mock data
          setTimeout(() => {
            setData(mockGtkData);
            setIsLoading(false);
          }, 800);
  ...
  const filteredData = useMemo(() => {
    ...
    switch (activeTab) {
      case 'pensiun':
        if (selectedYear !== 'All') {
          filtered = filtered.filter(d => d.pensiun_tahun && d.pensiun_tahun.toString() === selectedYear.toString());
        }
      ...
      case 'kepangkatan':
        return filtered.filter(d => d.pangkat && d.pangkat !== '-').sort((a, b) => a.pangkat.localeCompare(b.pangkat));
      case 'status':
      case 'pengangkatan':
      case 'sebaran':
      default:
  ...
  const StatCard = ({ title, value, icon: Icon, colorClass, delay }) => (
    ...
    >
      ...
      <div>
        ...
        <h3 className="text-3xl font-bold text-gray-900 tracking-tight">{value || 0}</h3>
      </div>
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#f5f5f7] flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-gray-500 font-medium animate-pulse">Menghubungkan ke Database Supabase...</p>
      </div>
    );
  }
        ...
        <div className="p-6 border-t border-gray-100/50">
          <div className="bg-gray-50 p-4 rounded-2xl flex items-center space-x-3 group cursor-help relative">
            ...
            <div className="absolute bottom-full left-0 mb-2 w-48 p-3 bg-gray-900/90 backdrop-blur-md text-white text-xs rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none shadow-xl">
              Terkoneksi secara real-time via PostgreSQL Supabase.
            </div>
          </div>
        </div>
      </div>
            ...
            {activeTab !== 'dashboard' && (
              <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden flex flex-col" style={{ animation: `fadeIn 0.4s ease-out both` }}>
                          ...
                          {activeTab === 'status' && (
                            <TableCell>
                              <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                ...
                                row.status && row.status.includes('PPPK') ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                              }`}>
                                {row.status}
                              </span>
                            </TableCell>
                ...
                <div className="bg-gray-50/80 backdrop-blur-md px-6 py-4 border-t border-gray-100 text-xs text-gray-500 flex justify-between items-center rounded-b-3xl">
                  ...
                  <span>Diperbarui secara real-time dari Database Supabase</span>
                </div>
              </div>
            )}
